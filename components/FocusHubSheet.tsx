...
// ─── AI Focus generator ───────────────────────────────────────────────────────
function generateAiFocus(schedules: Schedule[], mode: ViewMode): AiFocusItem[] {
  ...
  if (topItems.length>0) {
    ...
    items.push({ type:'win', accent:'#00C896',
  ...
  if (critical.length>0) items.push({ type:'conflict', accent:'#FF3B30',
    title:`${critical.length} critical item${critical.length!==1?'s':''} require immediate attention`,
    body: critical.map(s=>`• ${s.title}  ${formatTime(s.start_time)}`).join('\n') });
  if (overloaded) items.push({ type:'conflict', accent:'#FF9F43',
    title:'Schedule overload detected',
    body:`You have ${pending.length} items${mode==='today'?' today':' this week'}. Consider rescheduling ${pending.length-6} lower-priority items.` });
  if (high.length>2&&!overloaded) items.push({ type:'suggestion', accent:'#00C6FF',
    title:`${high.length} high-priority items queued`,
    body: high.slice(0,3).map(s=>`• ${s.title}`).join('\n')+(high.length>3?`\n• +${high.length-3} more`:'') });
  ...
  if (pending.length===0&&completed.length===0) items.push({ type:'suggestion', accent:'#00C6FF',
    title: mode==='today'?'Nothing scheduled today':'Empty week detected',
    body:'Add tasks or schedule items to let PlanIQ help you stay on track.' });
  return items;
}
...
export default function FocusHubSheet({ open, onClose }: Props) {
  ...
  // ── Scroll lock — same pattern as Edit Profile ─────────────────────────────
  useEffect(() => {
    if (open) {
  ...
  const aiFocus      = generateAiFocus(schedules, mode);
  const dayName  = now.toLocaleDateString('en-US',{weekday:'long'});
  const dateStr  = now.toLocaleDateString('en-US',{month:'long',day:'numeric'});

  // ── Styles — all using CSS variables so themes work automatically ──────────
  ...
  const SHEET: React.CSSProperties = {
    ...
    // Use CSS vars — adapts to every theme automatically
    ...
    touchAction: 'pan-y',           // allow sheet to scroll, block horizontal
  };
  ...
  // Text uses var(--dark) = darkest theme text, var(--mid), var(--lite)
  // These are already defined to be readable on --surf in every theme
  const T_TITLE: React.CSSProperties = {
    fontSize:'22px', fontWeight:800,
    color: 'var(--dark)', lineHeight:1.2, margin:0,
  };
  const T_SUB: React.CSSProperties = {
    fontSize:'13px', color:'var(--mid)', marginTop:'3px',
  };
  const T_SEC: React.CSSProperties = {
    fontSize:'11px', fontWeight:700,
    letterSpacing:'.6px', textTransform:'uppercase' as const,
    color: 'var(--lite)', marginBottom:'10px',
  };
  const T_BODY: React.CSSProperties = {
    fontSize:'12px', lineHeight:1.6,
    color: 'var(--mid)', whiteSpace:'pre-line',
  };
  ...
  const TAB = (active: boolean): React.CSSProperties => ({
    ...
    border: active
      ? '1px solid var(--border2, rgba(124,106,240,0.35))'
  ...
  const AI_TITLE = (accent: string): React.CSSProperties => ({
    fontSize:'13px', fontWeight:700,
    color: accent,   // accent is always a vivid color, readable on both light+dark surf
    marginBottom:'6px',
  });
  ...
  const S_META: React.CSSProperties = {
    fontSize:'11px', color:'var(--lite)', marginTop:'2px',
  };

  const DIVIDER: React.CSSProperties = {
    height:'1px', background:'var(--border)', margin:'4px 20px 14px', flexShrink:0,
  };

  const QA_ROW: React.CSSProperties = {
    display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px',
    padding:'0 20px', paddingBottom:'max(20px, env(safe-area-inset-bottom, 20px))',
    flexShrink:0,
  };
  ...
  return (
    <div style={OVERLAY} onClick={onClose}>
      <div style={SHEET} onClick={e=>e.stopPropagation()}>
        ...
        {/* Scrollable content — overscrollBehavior:contain stops background movement */}
        <div style={SCROLL}>
          {loading ? (
            <div style={{ textAlign:'center', padding:'32px 0', color:'var(--lite)', fontSize:'14px' }}>
                ...
                <>
                  ...
                  {displayItems.slice(0,8).map(s=>(
                    <div key={s.id} style={S_ITEM}>
                      <div style={{ width:'3px', height:'36px', borderRadius:'2px', flexShrink:0,
                      ...
                      {!s.is_completed ? (
                        <button onClick={()=>markDone(s.id)} disabled={marking===s.id}
                          style={{ flexShrink:0, width:'28px', height:'28px', borderRadius:'50%',
